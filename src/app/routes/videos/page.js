  ...
  return (
    <>
      <main className={`${styles.main} ${styles.contentpage}`}>
        <section
          className={`${styles.newsContainer} ${localstyles.videosContainer}`}
        >
          {!isLoading && !error ? (
...
const Video = ({
  ...
  return (
    <div
      className={`${localstyles.w100} ${localstyles.videoBlock} ${
        !mobile && play && selectedVideo == index
          ? localstyles.playingVideo
          : localstyles.pauseVideo
      }`}
    >
      <svg
        onClick={() => {
      ...
      <video
        ref={videoRef}
        autoPlay={play}
        src={`${process.env.NEXT_PUBLIC_CONNECT}://${process.env.NEXT_PUBLIC_URL_API}${data.attributes.video.data.attributes.url}`}
