...
export default function Page({ params }) {
  ...
  return (
    <>
      ...
      <main
        className={`${styles.main} ${styles.contentpage} ${stylesJob.contenter}`}
      >
        ...
        <section className={`${stylesJob.jobsBigContainer}`}>
          {!data ? (
            ...
            data.data.map((item, index) => {
              return (
                <article
                  key={`newsBigKey_${index}_${lang}`}
                  className={`${stylesJob.jobsBlock}`}
                >
                  <div className={`${stylesJob.jobsIco}`}>
                    <Image
                      unoptimized
                      src={`https://${process.env.NEXT_PUBLIC_URL_API}${item.attributes.image.data.attributes.url}`}
                  ...
                  <div className={`${stylesJob.infoJobsText}`}>
                    ...
                    <button
                      onClick={() => {
                        if (!moduleWindow) setModule(true);
      ...
      {!data ? (
        ...
        data.data.map((item, index) => {
          return (
            <article
              key={`plusKey_${index}`}
              className={`${stylesJob.plusBlock}`}
            >
              <div className={`${stylesJob.plusIco}`}>
                <Image
                  src={`https://${process.env.NEXT_PUBLIC_URL_API}${item.attributes.icon.data.attributes.url}`}
                  alt={item.attributes.name}
                  fill
                />
              </div>
