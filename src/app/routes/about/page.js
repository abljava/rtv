...
export default function Page({ params }) {
  ...
  useEffect(() => {}, [data, moduleWindow]);

  return (
    <>
      <main className={`${styles.main} ${styles.contentpage}`}>
        <section
          className={`${styles.newsContainer} ${localstyles.aboutContainer}`}
        >
          {isLoading ? (
            <Loader />
          ) : data ? (
            <>
              <div
                className={`${!mobile ? localstyles.w50 : null} ${
                  localstyles.text0
                }`}
              >
                ...
                <div
                  dangerouslySetInnerHTML={{
                    __html: data.data.attributes?.text,
                  }}
                ></div>
              </div>
              ...
              <div className={`${localstyles.w100} ${localstyles.text2}`}>
                ...
                <div>
                  <img
                    ...
                    alt={
                      data.data.attributes?.mapImage.data.attributes
                        .alternativeText || "Карта представительств"
                    }
                  />
                </div>
              </div>
