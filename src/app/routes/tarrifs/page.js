...
export default function Page({}) {
    ...
    const [selectType, setSelectType] = useState((params.get('select')) ? Number.parseInt(params.get('select')) : 0);
    ...
    return (
        <>
            ...
            <main className={`${styles.main} ${styles.contentpage} ${localstyles.tarrifPage}`}>
                ...
                <section className={`${localstyles.tarrifsContainer}`}>
                    ...
                    <div className={`${localstyles.leftBlock}`}>
                        <div className={`${localstyles.upNav}`}>
                            {
                                (!isLoading) ?
                                    (data && typeof data.data != "undefined" && Array.isArray(data.data.attributes.tarrifs.data)) ?
                                        ...
                                        if(index === selectType) {
                                            return(
                                                <>
                                                    ...
                                                    <div className={''} dangerouslySetInnerHTML = {{__html: item.attributes.fullText}}></div>
                                                </>
                                            )
                                        }
                                    })
