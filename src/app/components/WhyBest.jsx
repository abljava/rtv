            ...
            <div className = {`${styles.downBlock}`}>
                {
                        (!data) ? (<Loader />)
                          ...
                          data.data.map((item, index) => {
                            return(
                                <article key = {`whybestKey_${index}`} className = {`${styles.bestBlock}`}>
                                    <div className = {`${styles.oneBlock}`}>
                                        <Image unoptimized src = {`${process.env.NEXT_PUBLIC_CONNECT}://${process.env.NEXT_PUBLIC_URL_API}${item.attributes.icon.data.attributes.url}`} fill />
