        ...
        data.data ? (
          data.data.map((slide, index) => {
            return (
              <article
                style={
                  index === 0 ? { marginLeft: `-${selectSlide * 100}%` } : null
                }
                onMouseMove={() => {
                  setSliderSpeed(500000);
                }}
                onMouseLeave={() => {
                  setSliderSpeed(3000);
                }}
                ...
                <div className={`${styles.sliderBg}`}>
                  {typeof slide.attributes.image != "undefined" ? (
                    <Image
                      ...
                      src={
                        !image
                          ? `${process.env.NEXT_PUBLIC_CONNECT}://${process.env.NEXT_PUBLIC_URL_API}${slide.attributes.image.data.attributes.url}`
                          : image
                      }
                      fill
                    />
