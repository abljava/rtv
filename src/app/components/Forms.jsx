...
export const Forms = ({header = '', job = '', type = 'main'}) => {
 ...
 const telLang = {
  ...
  const handleSubmit = async (e) => {
    ...
    const request = await fetch(`${process.env.NEXT_PUBLIC_CONNECT}://${process.env.NEXT_PUBLIC_CALLBACK}/`, {
            method: "POST",
            headers: {
                ...
                (!sendStatus) ?
                    <>
                        ...
                        <input type="hidden" name="to"
                               value={(process.env.NEXT_PUBLIC_JOBSMAL) ? process.env.NEXT_PUBLIC_JOBSMAL : null}/>
        ...
        <form onSubmit = {(!statusSending) ? handleSubmit : exit} className = {`${styles.sliderForm}`} ref = {formRef}>
            {
                (!sendStatus) ?
                    <>
                        ...
                        {
                            (!mobile) ? <h3>{sliderHeaderLang[lang]}</h3> : null
                        }
          ...
          <div className = {styles.mainContainer}>
                ...
                <div className = {styles.cityesContainer}>
                    <input type = "text" name = "cityFrom" placeholder = {cityFromLang[lang]} />
                    {(!mobile) ? <Image src={'/beetween.svg'} alt={'Иконка Между городами'} fill /> : null }
                    <input type = "text" name = "cityTo" placeholder = {cityToLang[lang]} />
                </div>
                <div className = {styles.dataContainer}>
                    <div className={styles.dataRow}>
                        <input type = "text" name = "weight" placeholder = {weightLang[lang]} />
                        {(!mobile) ? <Image src={'/kg.svg'} alt={'Иконка Между городами'} fill /> : null }
                    </div>
                    <div className={styles.dataRow}>
                        <input type = "text" name = "volume" placeholder = {volumeLang[lang]}/>
                        {(!mobile) ? <Image src={'/volume.svg'} alt={'Иконка Между городами'} fill /> : null }
                    </div>
                </div>
          </div>

          <input type = "hidden" name = "to" value = {(process.env.NEXT_PUBLIC_JOBSMAL) ? process.env.NEXT_PUBLIC_JOBSMAL : null} />
          {       
                 (canSubmit) ?
                              <button onClick={f => f} type="submit">{(!statusSending) ? localeButton[lang] : loadingButton[lang] }</button>
                              : 
    ...
    case "main":
      return(
        <div className={`${styles.mainPageForm}`}>
          ...
          <Image src={`${ (!mobile) ? mainBg.src : mainBgMobile.src }`} alt = {`Доставка грузов по всему миру`} fill/>
